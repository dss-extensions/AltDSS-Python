...
def create_ckt13_verbose_with_iterators(altdss: AltDSS):
    altdss.ClearAll()
    altdss('new circuit.IEEE13Nodeckt')
    altdss.Settings.AdvancedTypes = True
    ...
    # Here we test the temporary iterator objs. Here, e.g., instead of creating a 
    # new actual Load object for each `Load.new`, an iterator object is reused
    # behind the scenes. If we wanted to keep a reference to a certain Load,
    # we would need to explicitly copy it (`from copy import copy; l671_ref = copy(l671)`)
    #
    # Note that this is a test and there won't be a performance difference for 
    # such a small circuit.
    with altdss.Settings.map_to_iterators():
        # Loads
        l671: Load = altdss.Load.new('671')
        l671.Bus1 = '671.1.2.3'
        l671.Phases = 3
...
def create_ckt13_shortcut_with_iterators(altdss: AltDSS, use_edit_method: bool):
    altdss.ClearAll()
    altdss('new circuit.IEEE13Nodeckt')
    altdss.Settings.AdvancedTypes = True
...
def test_create_ckt13_verbose(it: bool = False):
    '''
    ...
    if it:
        ...
        create_ckt13_verbose_with_iterators(altdss)
    else:
        from timeit import timeit
...
def test_create_ckt13_verbose_with_iterators():
    test_create_ckt13_verbose(True)

def _test_create_ckt13_shortcut(use_edit_method: bool, use_it: bool = False):
    '''
    This is the more verbose version; we don't expect anyone to do this since
    the data is usually loaded from other sources... but we still need to test it.
    '''
    ...
    if use_it:
        create_ckt13_shortcut_with_iterators(altdss, use_edit_method)
    else:
...
def test_create_ckt13_shortcut_with_iterators():
    _test_create_ckt13_shortcut(False, True)
...
if __name__ == '__main__':
    ...
    test_create_ckt13_verbose(False)
    test_create_ckt13_verbose(True)
