...
class IVsource(IDSSObj, VsourceBatch):
    ...
    if TYPE_CHECKING:
        ...
        def batch(self, **kwargs) -> VsourceBatch: #TODO: add annotation to kwargs (specialized typed dict)
            """
            Creates a new batch handler of (existing) Vsource objects
            """
            return self._batch_cls(self._api_util, **kwargs)

        def __iter__(self) -> Iterator[Vsource]:
            yield from VsourceBatch.__iter__(self)

        
    def new(self, name: AnyStr, *, begin_edit: Optional[bool] = None, activate=False, **kwargs: Unpack[VsourceProperties]) -> Vsource:
        """
    ...
    def batch_new(self, names: Optional[List[AnyStr]] = None, *, df = None, count: Optional[int] = None, begin_edit: Optional[bool] = None, **kwargs: Unpack[VsourceBatchProperties]) -> VsourceBatch:
        """
