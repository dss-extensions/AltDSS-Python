...
class PVSystem(DSSObj, CircuitElementMixin, PCElementMixin, ElementHasRegistersMixin):
    ...
    def edit(self, **kwargs: Unpack[PVSystemProperties]) -> PVSystem:
        """
        Edit this PVSystem.
    ...
    def _get_DynOut(self) -> List[str]:
        return self._get_string_array(self._lib.Obj_GetStringArray, self._ptr, 47)

    def _set_DynOut(self, value: List[AnyStr], flags: enums.SetterFlags = 0):
        value, value_ptr, value_count = self._prepare_string_array(value)
        self._lib.Obj_SetStringArray(self._ptr, 47, value_ptr, value_count, flags)
...
class PVSystemBatch(DSSBatch, CircuitElementBatchMixin, PCElementBatchMixin):
    ...
    def edit(self, **kwargs: Unpack[PVSystemBatchProperties]) -> PVSystemBatch:
        """
        Edit this PVSystem batch.
    ...
    if TYPE_CHECKING:
        def __iter__(self) -> Iterator[PVSystem]:
    ...
    def _get_DynOut(self) -> List[List[str]]:
        return self._get_string_ll(47)

    def _set_DynOut(self, value: List[AnyStr], flags: enums.SetterFlags = 0):
        value, value_ptr, value_count = self._prepare_string_array(value)
        for x in self._unpack():
            self._lib.Obj_SetStringArray(x, 47, value_ptr, value_count, flags)
...
class IPVSystem(IDSSObj, PVSystemBatch):
    ...
    if TYPE_CHECKING:
        ...
        def batch(self, **kwargs) -> PVSystemBatch: #TODO: add annotation to kwargs (specialized typed dict)
            """
            Creates a new batch handler of (existing) PVSystem objects
            """
            return self._batch_cls(self._api_util, **kwargs)

        def __iter__(self) -> Iterator[PVSystem]:
            yield from PVSystemBatch.__iter__(self)

        
    def new(self, name: AnyStr, *, begin_edit: Optional[bool] = None, activate=False, **kwargs: Unpack[PVSystemProperties]) -> PVSystem:
        """
        Creates a new PVSystem.
    ...
    def batch_new(self, names: Optional[List[AnyStr]] = None, *, df = None, count: Optional[int] = None, begin_edit: Optional[bool] = None, **kwargs: Unpack[PVSystemBatchProperties]) -> PVSystemBatch:
        """
        Creates a new batch of PVSystem objects
