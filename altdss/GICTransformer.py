...
class GICTransformer(DSSObj, CircuitElementMixin, PDElementMixin):
    ...
    def edit(self, **kwargs: Unpack[GICTransformerProperties]) -> GICTransformer:
        """
...
class GICTransformerBatch(DSSBatch, CircuitElementBatchMixin, PDElementBatchMixin):
    ...
    def edit(self, **kwargs: Unpack[GICTransformerBatchProperties]) -> GICTransformerBatch:
        """
    ...
    if TYPE_CHECKING:
        def __iter__(self) -> Iterator[GICTransformer]:
...
class IGICTransformer(IDSSObj, GICTransformerBatch):
    ...
    if TYPE_CHECKING:
        ...
        def batch(self, **kwargs) -> GICTransformerBatch: #TODO: add annotation to kwargs (specialized typed dict)
            """
            Creates a new batch handler of (existing) GICTransformer objects
            """
            return self._batch_cls(self._api_util, **kwargs)

        def __iter__(self) -> Iterator[GICTransformer]:
            yield from GICTransformerBatch.__iter__(self)

        
    def new(self, name: AnyStr, *, begin_edit: Optional[bool] = None, activate=False, **kwargs: Unpack[GICTransformerProperties]) -> GICTransformer:
        """
    ...
    def batch_new(self, names: Optional[List[AnyStr]] = None, *, df = None, count: Optional[int] = None, begin_edit: Optional[bool] = None, **kwargs: Unpack[GICTransformerBatchProperties]) -> GICTransformerBatch:
        """
