...
class EnergyMeter(DSSObj, CircuitElementMixin, EnergyMeterObjMixin, ElementHasRegistersMixin):
    ...
    def edit(self, **kwargs: Unpack[EnergyMeterProperties]) -> EnergyMeter:
        """
...
class EnergyMeterBatch(DSSBatch, CircuitElementBatchMixin, EnergyMeterBatchMixin):
    ...
    def edit(self, **kwargs: Unpack[EnergyMeterBatchProperties]) -> EnergyMeterBatch:
        """
    ...
    if TYPE_CHECKING:
        def __iter__(self) -> Iterator[EnergyMeter]:
...
class IEnergyMeter(IDSSObj, EnergyMeterBatch, IEnergyMeterMixin):
    ...
    if TYPE_CHECKING:
        ...
        def batch(self, **kwargs) -> EnergyMeterBatch: #TODO: add annotation to kwargs (specialized typed dict)
            """
            Creates a new batch handler of (existing) EnergyMeter objects
            """
            return self._batch_cls(self._api_util, **kwargs)

        def __iter__(self) -> Iterator[EnergyMeter]:
            yield from EnergyMeterBatch.__iter__(self)

        
    def new(self, name: AnyStr, *, begin_edit: Optional[bool] = None, activate=False, **kwargs: Unpack[EnergyMeterProperties]) -> EnergyMeter:
        """
    ...
    def batch_new(self, names: Optional[List[AnyStr]] = None, *, df = None, count: Optional[int] = None, begin_edit: Optional[bool] = None, **kwargs: Unpack[EnergyMeterBatchProperties]) -> EnergyMeterBatch:
        """
